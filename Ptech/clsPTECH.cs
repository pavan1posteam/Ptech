...
namespace PTech
{
    class clsPTECH
    {
        ...
        public class PtechPos
        {
            ...
            //        // Add body (even though it's a GET, as required by PTECH)
            //        var body = new
            //        {
            //            PositiveOnly = true,
            //            WithNegative = false
            ...
            //            productList = result?.Data ?? new List<Datum>();
            //        }
            //        else
            //        {
            //            Console.WriteLine($"API Error: {response.StatusCode} - {response.ErrorMessage}");
            //        }
            //        File.AppendAllText("12271(1).json", response.Content);
            //    }
            //    catch (Exception ex)
            //    {
            //        Console.WriteLine("PTECH API Error: " + ex.Message);
            //    }
            ...
            public void PtechcsvConverter(int storeid, decimal tax, string BaseUrl, string Username, string Password, bool IsMarkUpPrice, int MarkUpValue)
            {
                ...
                try
                {
                    ...
                    foreach (var item in productList)
                    {
                        ...
                        {
                            pdf.sprice = Convert.ToDecimal(item.PRICEB);
                        }
                        pdf.Start = "";
                        pdf.End = "";
                        pdf.Tax = tax;
                    ...
                    if(storeid == 12561)
                    {
                        Username = "b3HmA6i2bVlDBwA/b3aPilVlzQesowmuBJ9HeBnZxTek2K9mCZb3fQ==";
                        Password = "3NO+tiS1ll2X9CTczrKND4adgKLdPDytfwZonzP03Swx9PaPQTTGXQ==";
                        productList = Products(storeid, tax, BaseUrl, Username, Password);
                        foreach (var item in productList)
                        {
                            datatableModel pdf = new datatableModel();
                            pdf.StoreID = storeid;
                            ...
                            pdf.StoreProductName = b.Trim();
                            pdf.StoreDescription = b.Trim();
                            pdf.Price = Convert.ToDecimal(item.REGPRICE);
                            pdf.Start = "";
                            pdf.End = "";
                            pdf.Tax = tax;
        ...
        #region SYPRAM SOFTWARE POS
        //public class PTECHCsvProducts //SYPRAM SOFTWARE
        //{
        ...
        //                    foreach (var item in productList)
        //                    {
        //                        if(Small_Case.Contains(storeid.ToString()))
        //                        {
        ...
        //                                pdf.pack = itm["packname"].ToString();
        ...
        //                                pdf.Tax = tax;
        //                                if(pdf.pack.Contains("12Pk") && pdf.uom.Contains("750"))
        //                                {
        //                                    pdf.Price = pdf.Price - (pdf.Price * 15 / 100);
        //                                }
        //                                else if (pdf.pack.Contains("6Pk") && pdf.uom.Contains("1.5L"))
        //                                {
        //                                    pdf.Price = pdf.Price - (pdf.Price * 10 / 100);
        //                                }
        ...
        //                                {
        //                                    string pack = itm["PackName"].ToString();
        //                                    if (string.IsNullOrEmpty(pack))
        //                                        pdf.pack = 1.ToString();
        //                                    else
        //                                        pdf.pack = getpack(pack).ToString();
        //                                }
        //                                else
        //                                {
        //                                    pdf.pack = 1.ToString();
        ...
        //    {
        //        pack = pack.ToUpper();
        //        var regexMatch = Regex.Match(pack, @"(?<Result>\d+)PK");
